$(document).ready(function () {
  ...
  for (timeslot = 7; timeslot <= 19; timeslot++) {
    ...
    var showHours = 0;
    showHours = moment().hour(timeslot).minutes(0).format("HH:mm a");
    ...
    console.log(timeslot);
    if (timeslot === moment().hour()) {
      $organiser.addClass('present');
    }
    else if (timeslot < moment().hour()) {

      $organiser.addClass('past');
    }
    else if (timeslot> moment().hour()) {
      $organiser.addClass('future');
    }
    $weekly_organiser.append($organiser);
  ...
  $('.icon').click(function (e) {
    ...
    var $index = $(this ).attr('event-save');
